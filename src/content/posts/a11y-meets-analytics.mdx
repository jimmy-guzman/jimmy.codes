...
When working on analytics, I've come across different implementations that tracked clicks using data attributes, like `data-track-name`, or even simple IDs. These approaches had similar issues and were brittle, prone to breaking during refactors, and required tedious manual work, which wasn't great for developer experience. In one project, we were also on a mission to improve accessibility, so I wanted a solution that aligned with that goal.
...
We also had multiple "Edit" buttons across different sections that all logged the same name, making it impossible to distinguish which content was being edited. This pushed us to use `aria-labelledby` to reference section headings, creating unique names like "Edit Project Alpha" and "Edit User Profile".
