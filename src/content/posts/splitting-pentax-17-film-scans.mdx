...
When I started getting into film photography, I didn't make it easy on myself. I picked up the **Pentax 17**, a half-frame camera that captures two 17x24 mm photos on a single 35 mm frame.
...
I wanted a faster and more reliable process, so I built an automated solution using [**Node.js**](https://nodejs.org/en) and [**Sharp**](https://sharp.pixelplumbing.com). It splits each scan into two clean frames and optimizes their dimensions to preserve quality. Along the way, I learned more than expected about signal processing, image geometry, and compression.
...
My local lab returns each film scan as a single JPEG. Each contains:
...
Because these scans are already JPEGs, preserving quality during re-encoding is important:
...
Processing 112 images (224 frames) produced the following:
