...
Feed this to analytics and you've reported zero demand for a product that's actually out of stock. Ship it to inventory and you've marked sold-out items as available. The downstream systems never crash, they just quietly make wrong decisions with confident data.
...
For production code, libraries like [`tiny-invariant`](https://github.com/alexreardon/tiny-invariant) provide battle-tested assertion helpers that can be stripped in production builds for zero runtime cost.

This isn't as good as parsing, but it's infinitely better than defaulting. You're still throwing exceptions mid-execution instead of failing at the boundary.
