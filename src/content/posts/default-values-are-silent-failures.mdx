...
```ts twoslash
function getVolume() {
  return localStorage.volume || 0.5
}
```
...
Not quite. We fixed a `||` bug and introduced a domain bug.
...
Completely different scenarios. Identical output.

If this feeds inventory or analytics, your numbers are confidently wrong.
...
// ---cut---
// Defaulting: validation without evidence
function processOrder(items: OrderItem[]) {
  const quantity = items[0]?.quantity ?? 0
  // quantity is a number, but is it real?
}
```
...
// ---cut---
// Parsing: validation with evidence
function parseOrderItem(item: OrderItem): Result<ValidItem, string> {
...
1. **Is this value truly optional in the domain?**  
   Can a product with no price be sold? Is an order with no items still an order?
...
// ---cut---
// User preference with genuine default
...
Catch bad data where it enters your system:
...
app.post('/orders', (req, res) => {
  ...
  createOrder(result.data) // ValidOrder, not OrderItem
})
...
In those cases, use assertions instead of defaults:
...
function processOrder(item: OrderItem) {
  invariant(item.quantity, 'quantity is required')
  invariant(item.quantity > 0, 'quantity must be positive')

  // Now quantity is guaranteed non-zero
...
function processOrder(item: OrderItem) {
  const quantity = item.quantity ?? 0
  const itemsPerBox = quantity / item.boxSize // might be 0 / boxSize
}
```

Assertions crash immediately with context. Defaults silently propagate bad data.

For TypeScript, assertion functions narrow types:

```ts twoslash
function assertPositive(value: number | undefined): asserts value is number {
  if (!value || value <= 0) {
...
function processOrder(item: OrderItem) {
  assertPositive(item.quantity)
  // TypeScript knows quantity is number, not number | undefined
  const itemsPerBox = item.quantity / item.boxSize
  //                      ^?
}
```

This isn't as good as parsing—you're still throwing exceptions mid-execution instead of failing at the boundary. But it's infinitely better than defaulting.
