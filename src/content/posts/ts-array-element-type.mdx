...
However, this breaks down if the property is optional and `strictNullChecks` is enabled.
...
We need to filter out non-array types before indexing. But we also want to keep our utility working for raw arrays from Step 1. Here's how we do both:
...
This approach shines in practical scenarios where you need type safety without duplication.
...
The tuple syntax `[K] extends [never]` is important—it prevents TypeScript from distributing the check over union types, which would break the logic.
...
2. Filter to arrays only: `Extract<..., readonly unknown[]>` → `{ id: string; total: number }[]`
3. Get the element type: `[number]` → `{ id: string; total: number }`

By filtering out `undefined` (and any other non-array types) before indexing, we avoid the type error.
