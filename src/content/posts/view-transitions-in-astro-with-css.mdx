...
When I first heard about the [View Transitions API](https://developer.mozilla.org/en-US/docs/Web/API/View_Transition_API), I assumed it only worked for single page apps.  
Most examples relied on frameworks or client-side routers. I wanted the same polish for a static site built with [Astro](https://astro.build).

It turns out you can get smooth, page-to-page transitions with no JavaScript at all. Just CSS.

---

## Why I Tried This

Astro already makes navigation feel instant by streaming pages as static HTML, but I wanted something more, a subtle fade that connects pages without feeling flashy.
...
The browser can detect when you move between same-origin pages (pages on the same domain—like going from `/blog` to `/about` on `jimmy.codes`) and automatically animate the visual change.  
...
- **Reduced motion**
  The `prefers-reduced-motion` query turns everything off for users who opt out of animation.
  It also disables navigation transitions entirely to avoid any motion between pages.
...
The next step is telling the browser _which_ elements should carry over between pages.
...
Everything moves together—simpler, but less hierarchy.

These 30-50ms differences are subtle but shape whether motion feels intentional or mechanical.
