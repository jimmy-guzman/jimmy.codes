...
This error appears as: **“Element implicitly has an 'any' type because expression of type 'string' can't be used to index type”** and commonly shows up as TypeScript error **TS7053** when using dynamic string keys.

### Quick Fix

If you control the key, change the function to accept `keyof typeof item`.  
If the key is dynamic, use a type guard or runtime check.  
...
Here, `key` is of type `string`, which means it could be any string value. TypeScript cannot guarantee that this string corresponds to a valid key in the `item` object, which only has the keys `name` and `type`. Therefore, it raises an error to prevent potential runtime issues.
...
### 6. Add a permissive index signature

While not recommended, you can add a permissive index signature to your interface or type.
...
This approach is generally discouraged as it defeats the purpose of TypeScript's type safety since it essentially disables type checking for property access.
...
- Be cautious with type assertions since they bypass safety checks
