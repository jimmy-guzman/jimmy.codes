...
## Why this is a design problem, not a null problem

The issue is not that the function returns `null`.

The issue is that callers must understand **why** it returns `null` in order to use it correctly.
...
That check is now required everywhere.
...
Those are business rules, and they escaped the function.
...
function calculateShipping(weight: number, distance: number) {
  if (weight > 100) {
    return null // use freight instead
  }
...
Now every caller must know:

- What `null` means
- What the weight threshold is
- What to do instead

Better:

```ts
function calculateStandardShipping(weight: number, distance: number) {
  return weight * distance * 0.1
}

function calculateFreightShipping(weight: number, distance: number) {
  // different calculation
}

function calculateShipping(weight: number, distance: number) {
  return weight > 100
    ? calculateFreightShipping(weight, distance)
    : calculateStandardShipping(weight, distance)
}
```

The routing decision is explicit and owned by `calculateShipping`.

Calculation functions calculate. They don't route.
...
async function getUser(id: string) {
  if (id.startsWith("admin_")) {
    throw new Error("use getAdminUser instead")
  }
...
Now callers must pre-route:

```ts
const user = id.startsWith("admin_")
...
One decision point. One API. No leaks.
