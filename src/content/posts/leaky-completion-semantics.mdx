...
Most people frame this as a Single Responsibility Principle violation. That framing is not wrong, but it does not quite name the shape of the problem. The function is both deciding whether work should happen and doing the work. `getOrder` claims to fetch an order. It also secretly decides which orders are allowed to be returned. A function should not quietly do both unless routing is what the name promises.
...
**Would this behavior surprise someone who only read the function name?** The tell is when you check conditions before calling a function, or when a code review reveals a special return value you did not know about. The surprise is the design smell.
